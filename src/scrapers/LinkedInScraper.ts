...
export class LinkedInScraper extends BaseScraper {
  ...
  scrape(data: JobData, config?: RemoteConfig): JobData {
    ...
    const detailContainer = this._queryFirst([
  ...
  private _scrapeDetailContainer(
    data: JobData,
    detailContainer: Element,
    dynamicSelectors: RemoteConfig['linkedin'],
  ): void {
    ...
    if (!companyEl) {
      ...
      if (ariaEl) {
        ...
        const link =
          ariaEl.querySelector('a[href*="/company/"]') || ariaEl.closest('a[href*="/company/"]');
  ...
  private _scrapeSingleJobPage(data: JobData, dynamicSelectors: RemoteConfig['linkedin']): void {
    ...
    if (companyEl) {
      ...
      if (!text && companyEl.getAttribute('aria-label')) {
        text = (companyEl.getAttribute('aria-label') || '')
          .replace('Company, ', '')
          .replace('.', '');
      }
      data.company = text;
  ...
  private _scrapeLDJson(data: JobData): void {
    if (!data.appLink) {
      try {
        ...
        console.log('Error parsing LD-JSON', e);
      }
    }
  }
}
