  ...
  for (let attempt = 0; attempt <= config.maxRetries; attempt++) {
    try {
      ...
      if (attempt < config.maxRetries) {
        ...
        console.warn(
          `API request failed (${response.status}), retrying in ${delay}ms... (attempt ${attempt + 1}/${config.maxRetries})`,
        );
        await sleep(delay);
        continue;
      }
...
export function getReadableError(
  ...
  if (source === 'Notion') {
    if (status === 401)
      return 'Notion integration secret is invalid. Check your settings in Options.';
    if (status === 403)
      return 'Notion database is not shared with your integration. Open the database → "..." → "Connect to" → select your integration.';
    if (status === 404) return 'Notion database not found. Check the Database ID in Options.';
    if (status === 400)
      return `Notion rejected the data: ${message || 'Check that your database properties match the required schema.'}`;
  }
